...
searches:
  - id: tehran-apt-40-60          # شناسه یکتا (انگلیسی)
    ...
    # جغرافیا = allowlist محله‌ها (روی district_persian پاسخ — ADR-0006)؛ فقط «گیت نوتیف» است:
    # آگهی خارج از لیست هم جزئیات/امتیاز می‌گیرد، فقط پیام نمی‌گیرد (اصلاح ۲۰۲۶-۰۹-۱۸)
    districts: [شهران, پونک]       # تطبیق contains نرمال‌شده: «شهران» ← «شهران جنوبی/شمالی»
    allow_unknown_district: false  # آگهیِ بدون محله: نوتیف نمی‌گیرد (پیش‌فرض) — ولی پردازش/انبار کامل می‌شود
...
- **جغرافیا با `districts` (ADR-0006؛ اصلاح معنا ۲۰۲۶-۰۹-۱۸):** پس‌فیلتر سمت ما روی `district_persian` پاسخ که فقط **گیت نوتیف** است. تطبیق **contains** روی متن نرمال‌شده است (تبدیل ي/ك عربی به ی/ک فارسی، یکسان‌سازی نیم‌فاصله با فاصله، حذف فاصله‌های اضافی) — پس «نارمک» هر دو «نارمک» و «نارمک جنوبی» را می‌گیرد. آگهی خارج از لیست (یا بدون محله وقتی `allow_unknown_district: false`) **جزئیات و امتیاز می‌گیرد و کامل ذخیره می‌شود** (شهروند کامل انبار همتا) — فقط **نوتیف** نمی‌گیرد و ردیف delivery نمی‌سازد؛ تکرارنشدن در poll بعدی از token یونیک می‌آید، نه از این گیت.
...
- ارسال مجدد از تاریخ (`/resend`؛ تاریخ شمسی): همهٔ ممتازهای از آن تاریخ به **فقط همین کاربر**، سقف ۳۰ روز، مستقل از جدول delivery؛ **«ممتاز» = وجود ردیف delivery برای آن آگهی** (اصلاح ۲۰۲۶-۰۹-۱۸ — تصمیم ۱۳؛ آگهیِ ردشده از `min_score`/allowlist ردیف ندارد و `/resend` هم نمی‌شود)؛ cooldown ‏۱۰ دقیقه (بخش ۳.۱).
...
polling:
  ...
  fetch_post_detail: true         # GET posts-v2/web برای همهٔ آگهی‌های جدید (فارغ از allowlist — اصلاح ۲۰۲۶-۰۹-۱۸، ADR-0009)
```
