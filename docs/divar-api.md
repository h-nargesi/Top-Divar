...
تصمیم (ADR-0004 + ADR-0006 + ADR-0009): پایش و تشخیص جدید با `postlist/w/search`؛ جغرافیا با **allowlist محله‌ها** روی `district_persian` پاسخ search (پس‌فیلتر سمت کلاینت — بخش ۵). فیلدهای ملک (متراژ، سال ساخت، اتاق، قیمت‌متری، طبقه، امکانات) از **جزئیات آگهی** می‌آیند چون در کارت search نیستند (بخش ۷.۳). استفاده از `bbox` رد شد (تصمیم 2026-09-17). اگر در آینده قاعده امتیاز جغرافیایی (مثل فاصله تا مترو) لازم شد، `viewport` می‌تواند به‌عنوان Fetcher مکمل برگردد.
...
- **جغرافیا (ADR-0006):** مکانیزم اصلی، پس‌فیلتر `districts` روی `district_persian` پاسخ است (`configuration.md` بخش ۱). دیوار خودش هم کلید بومی `districts` (با ID عددی محله‌ها) دارد — بخش ۴ — که با جدول نگاشت نام→ID (خودکار از جزئیات + دستی از CLI — ADR-0006، بخش ۱۲.۵) به‌تدریج جای پس‌فیلتر را می‌گیرد. استفاده از `bbox` منتفی است (تصمیم 2026-09-17؛ پیش‌تر پیش‌فیلتر اختیاری سرور بود).
...
7. **جزئیات (ADR-0009):** برای آگهی **جدید** که از جغرافیا عبور کرده → `GET /v8/posts-v2/web/{token}` (شکست → fallback HTML `https://divar.ir/v/{token}`). این درخواست هم مشمول `polling.detail_min_interval` است.
...
- حداقل فاصلهٔ درخواست‌ها **به تفکیک نوع و مستقل**: `polling.search_min_interval` و `polling.detail_min_interval` (پیش‌فرض هر دو `30s`؛ رفتار واقعی rate limit دادهٔ رسمی ندارد و در اجرا کشف می‌شود → هر دو در تنظیمات قابل ویرایش‌اند — `configuration.md` بخش ۴)؛ بازه poll هر جستجو: حداقل `5m` + jitter تصادفی. اجرا همچنان یک صف سریال واحد است (concurrency = ۱).
...
- [ ] **رفتار rate limit** — در حین اجرا از لاگ: هر 429/5xx با هدرهای `Retry-After` ثبت و در اینجا مستند می‌شود (بخش ۱۰)؛ مقدار بهینهٔ `search_min_interval` / `detail_min_interval` هم از همین داده کالیبره می‌شود
