...
تصمیم (ADR-0004 + ADR-0006 + ADR-0009؛ اصلاح جغرافیا ۲۰۲۶-۰۹-۱۸): پایش و تشخیص جدید با `postlist/w/search`؛ جغرافیا فقط از فیلتر بومی دیوار داخل `form_data` (از جمله `districts` با شناسهٔ عددی اگر کاربر کپی کند). فهرست نام محله و گیت نوتیف جغرافیایی حذف شدند. همهٔ آگهی‌های جدید جزئیات و امتیاز می‌گیرند (بخش ۹). فیلدهای ملک (متراژ، سال ساخت، اتاق، قیمت‌متری، طبقه، امکانات) از **جزئیات آگهی** می‌آیند چون در کارت search نیستند (بخش ۷.۳). استفاده از `bbox` رد شد (تصمیم 2026-09-17). اگر در آینده قاعده امتیاز جغرافیایی (مثل فاصله تا مترو) لازم شد، `viewport` می‌تواند به‌عنوان Fetcher مکمل برگردد.
...
- **جغرافیا (ADR-0006 اصلاح ۲۰۲۶-۰۹-۱۸):** فقط کلیدهای `form_data` که کاربر از دیوار کپی کرده — از جمله `districts` با شناسهٔ عددی (بخش ۴). برنامه شناسه تزریق نمی‌کند و پس‌فیلتر روی `district_persian` ندارد. استفاده از `bbox` منتفی است (تصمیم 2026-09-17).
...
| `districts` | `repeated_string` | `{"repeated_string": {"value": ["198", "399", …]}}` | **فیلتر بومی محله‌ها سمت سرور** با ID عددی؛ کاربر از DevTools کپی می‌کند. برنامه تزریق نمی‌کند (ADR-0006 اصلاح ۲۰۲۶-۰۹-۱۸). شناسه‌های نمونه: یوسف‌آباد = `90`، هروی = `1024` |
...
- به‌ازای هر آگهی (`POST_ROW`) — **فیلدست کامل با نمونه دوصفحه‌ای بدون کوکی تأیید شد**:
  ...
  - `data.action.payload.web_info.district_persian` / `city_persian` — محله/شهر برای نمایش و قاعدهٔ امتیاز `contains_any`؛ گیت نوتیف جغرافیایی نیست (ADR-0006 اصلاح ۲۰۲۶-۰۹-۱۸)
...
6. فیلتر جغرافیایی سمت ما وجود ندارد (ADR-0006 اصلاح ۲۰۲۶-۰۹-۱۸). seen بودن از تشخیص تکراری (`token`) می‌آید.
7. **جزئیات (ADR-0009):** برای **همهٔ آگهی‌های جدیدِ poll** → `GET /v8/posts-v2/web/{token}` (شکست → fallback HTML `https://divar.ir/v/{token}`). این درخواست هم مشمول `polling.detail_min_interval` است؛ صف جزئیات **FIFO** است. **هزینهٔ نرخ تابع پهنای فیلتر جستجوست:** جستجوی پهن‌تر = آگهی جدید بیشتر در هر poll = درخواست جزئیات بیشتر (محدود به `max_pages_per_poll` و فاصلهٔ `detail_min_interval`).
8. نرمال‌سازی (بخش ۸) → امتیازدهی (خط لولهٔ دو مرحله‌ای: اول ذخیرهٔ همه، بعد امتیازدهی دسته — معماری «جریان داده») → در صورت عبور از `min_score`، نوتیف.
...
- حداقل فاصلهٔ درخواست‌ها **به تفکیک نوع و مستقل**: `polling.search_min_interval` و `polling.detail_min_interval` (پیش‌فرض هر دو `30s`؛ رفتار واقعی rate limit دادهٔ رسمی ندارد و در اجرا کشف می‌شود → هر دو در تنظیمات قابل ویرایش‌اند — `configuration.md` بخش ۴)؛ **`detail_min_interval` قلم اول کالیبراسیون از لاگ اجراست (اصلاح ۲۰۲۶-۰۹-۱۸)** — چون همهٔ آگهی‌های جدید جزئیات می‌گیرند، حجم درخواست جزئیاتِ هر poll تابع پهنای فیلتر جستجوست و اولین عددی است که باید از لاگ 429/5xx تنظیم شود؛ بازه poll هر جستجو: حداقل `5m` + jitter تصادفی. اجرا همچنان یک صف سریال واحد است (concurrency = ۱)؛ صف جزئیات FIFO است.
...
- [x] ~~جدول نگاشت نام محله → `districts` ID~~ — از MVP خارج شد (ADR-0006 اصلاح ۲۰۲۶-۰۹-۱۸): کاربر شناسه را از DevTools در `form_data` می‌گذارد؛ برنامه تزریق و برداشت خودکار ندارد. شناسه‌های نمونه برای کپی دستی: یوسف‌آباد = `90`، هروی = `1024`
...
### ۱۲.۵. شناسهٔ محله در جزئیات (مرجع دستی — خارج از مسیر اجرا)

برداشت خودکار و جدول نگاشت از MVP خارج‌اند (ADR-0006 اصلاح ۲۰۲۶-۰۹-۱۸). مسیر زیر فقط برای کسی است که بخواهد شناسه را از یک پاسخ جزئیات بخواند و در `form_data` کپی کند:

- از `seo.bread_crumb[]` آیتمی که `name == seo.web_info.district_persian` **و** `search_data.form_data.data.districts` تک‌عضوی است → شناسهٔ همان محله (نمونهٔ `gammaxvi`: «هروی» → `1024`)
